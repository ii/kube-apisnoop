...
const (
	defaultAnnotation      = "initializer.kubernetes.io/tproxy"
	defaultInitializerName = "tproxy.initializer.kubernetes.io"
	defaultConfigmap       = "tproxy-initializer"
...
func main() {
	...
	flag.StringVar(&configmap, "configmap", defaultConfigmap, "The initializer configuration configmap")
	...
	if deployment.ObjectMeta.GetInitializers() != nil {
		...
		if initializerName == pendingInitializers[0].Name {
			...
			if requireAnnotation {
				...
				_, ok := a[annotation]
				log.Printf("Deployment Annotation: '%s'", annotation)
				if !ok {
					log.Printf("Required '%s' deployment annotation missing; skipping container injection", annotation)
			...
			// Inject the volume mounts and env vars into the existing containers.
